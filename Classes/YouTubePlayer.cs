...
namespace YouTube
{
    ...
    public class YouTubePlayer
    {
        ...
        private void SetupPlayerView(CGRect frame)
        {
            ...
            PlayerView.LoadStarted += PlayerView_LoadStarted;
            PlayerView.LoadFinished += PlayerView_LoadFinished;
        }

        private void PlayerView_LoadFinished(object sender, EventArgs e)
        {
            HideActivityIndicatorView();
        }

        private void HideActivityIndicatorView()
        {
        ...
        private void PlayerView_LoadStarted(object sender, EventArgs e)
        {
            if (activityIndicatorView == null)
            {
                activityIndicatorView = new UIActivityIndicatorView();
                PlayerView.Add(activityIndicatorView);
                activityIndicatorView.TintColor = UIColor.White;
            }
            activityIndicatorView.Center = PlayerView.Center;
        ...
        private void NotifyDelegateOfYouTubeCallbackUrl(NSUrl url)
        {
            ...
            if (action == OnReady)
            {
                PlayerViewReady?.Invoke(this, new EventArgs());
                HideActivityIndicatorView();
            }
            else if (action == OnStateChange)
            {
                PlayerStateChanged?.Invoke(this, this.GetPlayerStateFromString(data[1]));
            }
            else if (action == OnPlaybackQualityChange)
            {
                PlayerQualityChanged?.Invoke(this, this.GetPlaybackQualityFromString(data[1]));
            }
            else if (action == OnError)
            {
                PlayerReceivedError?.Invoke(this, this.GetPlayerErrorFromString(data[1]));
                HideActivityIndicatorView();
            }
        }
